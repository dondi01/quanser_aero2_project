classdef casadi_block_ < matlab.System & matlab.system.mixin.Propagates
    ...
    methods (Access = protected)
        ...
        function setupImpl(obj,~,~)
            ...
            % Model equations
            xdot = [x2; 1/Jp*( ...
            beta*(x4^2*sign(x4)*2)*Dt ...
            -C*x2 ...
            -Mb*g*Dm*sin(x1));
            0;
            1/0.1699*(15.2861*u-gamma*x4^2*sign(x4)-x4);x3-x1];

            % Objective term
            L =((x1-x3)^2+2*x2^2+0.5*x5^2);
            ...
            for k=0:N-1
                ...
                Xk = MX.sym(['X_' num2str(k+1)], 5);
                w = {w{:}, Xk};
                lbw = [lbw; -pi/2; -inf;-inf;-270;-inf];
                ubw = [ubw;  pi/2;  inf; inf;270;inf];
                w0 = [w0; 0; 0; 0; 0; 0];
